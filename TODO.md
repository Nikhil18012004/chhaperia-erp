...
- ~~**H12** two `quoteForm` functions; **M15** no Run Payroll button~~ — both already fixed on main by 2026-09-25 (one `quoteForm` remains, `mod-hr.js` has ▶ Run Payroll).
...
- ~~H1 simultaneous creates share a number~~ — CLOSED 2026-09-26: every document number (PO, SO, WO, GRN, lead, supplier, quotation, complaint, lab certificate, worker, leave, transporter, appointment, approval, incoming test) is taken from a per-series row under a lock (`counters` table, `repository.nextNumber`, `services/numbering.js`); `http.js` races 6 sales orders and 6 leads. Numbers are never reused, which also closed the first half of H3.
...
- ~~H3 work-order numbers reused after delete (certificate inherited)~~ closed with H1. **H4** still open: renumbering a work order loses its certificate and strands the sales-order batch that names the old number.
