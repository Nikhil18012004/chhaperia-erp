...
async function run() {
  ...
  section("Chatbot — asks are role-scoped, training is admin/office only");
  {
    ...
    /* --- intent keywords must match WHOLE words. "lab" hid inside "available"
           and "test" inside "latest", so ordinary stock questions were answered
           with QC statistics and the word "available" was unreachable. --- */
    const isLab = (r) => /Lab reports on file|waiting on a lab measurement/.test(r.d.answer || "");
    const avail = await call("POST", "/chatbot/ask", A, { q: "is mica tape available" });
    ok("'available' does not read as 'lab'", avail.status === 200 && !isLab(avail), (avail.d.answer || "").slice(0, 70));
    const latest = await call("POST", "/chatbot/ask", A, { q: "latest stock position" });
    ok("'latest' does not read as 'test'", latest.status === 200 && !isLab(latest), (latest.d.answer || "").slice(0, 70));
    const labels = await call("POST", "/chatbot/ask", A, { q: "how do I print labels" });
    ok("'labels' does not read as 'lab'", labels.status === 200 && !isLab(labels), (labels.d.answer || "").slice(0, 70));
    // …but a real plural still matches: "customers" must reach the customer intent
    ok("plural still matches its trigger", /Customers: \d+/.test(a1.d.answer));

    /* --- the bare words "so" and "po" are not order references --- */
    const soWord = await call("POST", "/chatbot/ask", C, { q: "we are low on mica so what should I order" });
    ok("the English word 'so' is not a sales order", soWord.status === 200 && !/Sales orders/.test(soWord.d.answer), (soWord.d.answer || "").slice(0, 70));
    const tempo = await call("POST", "/chatbot/ask", A, { q: "arrange a tempo for delivery" });
    ok("'tempo' is not a purchase order", tempo.status === 200 && !/Purchase orders: \d+ total/.test(tempo.d.answer), (tempo.d.answer || "").slice(0, 70));

    /* --- redacted data must report ABSENT, never a false all-clear. The coating
           supervisor runs the QC gate; telling them no batch owes a measurement
           when their view simply cannot see the list is misinformation. --- */
    const supLab = await call("POST", "/chatbot/ask", C, { q: "pending lab tests" });
    ok("supervisor lab ask says 'not available', not an all-clear",
      supLab.status === 200 && /isn't available/.test(supLab.d.answer) && !/No jobs are waiting/.test(supLab.d.answer),
      (supLab.d.answer || "").slice(0, 90));
    const admLab = await call("POST", "/chatbot/ask", A, { q: "pending lab tests" });
    ok("admin lab ask still answers for real", admLab.status === 200 && /Lab reports on file: \d+/.test(admLab.d.answer), (admLab.d.answer || "").slice(0, 70));

    /* --- a work order resolves however the number is typed (ids are padded to 4) --- */
    {
      const stA = await call("GET", "/state", A);
      const anyWo = (stA.d.workorders || [])[0];
      if (anyWo) {
        const digits = /(\d+)\s*$/.exec(String(anyWo.id));
        const bare = digits ? String(+digits[1]) : null;   // "0003" -> "3"
        const unpadded = await call("POST", "/chatbot/ask", A, { q: "status of WO " + bare });
        ok("an unpadded work-order number resolves ('WO " + bare + "' -> " + anyWo.id + ")",
          unpadded.status === 200 && unpadded.d.answer.indexOf(anyWo.id) === 0, (unpadded.d.answer || "").slice(0, 70));
        const helpStyle = await call("POST", "/chatbot/ask", A, { q: "WO-" + String(bare).padStart(3, "0") + " status" });
        ok("the help text's own 'WO-0NN status' form resolves",
          helpStyle.status === 200 && helpStyle.d.answer.indexOf(anyWo.id) === 0, (helpStyle.d.answer || "").slice(0, 70));
      } else ok("an unpadded work-order number resolves", true, "skipped — no work orders in this dataset");
    }
    ...
    /* --- a bulk upload is ALL-or-nothing. It used to write as it went, so a bad
           row halfway down left the earlier rows committed under an error that
           never said so — and the corrected re-upload saved them all again. --- */
    {
      const partial = await call("POST", "/chatbot/knowledge", A, { entries: [
        { question: "Row one", answer: "Fine." },
        { question: "Row two", answer: "Also fine." },
        { question: "Row three has no answer", answer: "   " },
        { question: "Row four", answer: "Fine too." },
      ] });
      ok("a bad row rejects the whole upload (400)", partial.status === 400, JSON.stringify(partial.d).slice(0, 90));
      ok("the error names the offending row", /entry 3 of 4/.test((partial.d && partial.d.error) || JSON.stringify(partial.d)), JSON.stringify(partial.d).slice(0, 120));
      const afterFail = await call("GET", "/chatbot/knowledge", A);
      ok("NOTHING was saved from the failed upload", afterFail.status === 200 && afterFail.d.length === 0, "KB holds " + (afterFail.d || []).length);
    }

    /* --- a one-word trained entry must not out-rank the live data. "stock" is
           contained in every stock question; the near-verbatim bonus alone used
           to push the canned answer past the cut-off. --- */
    {
      const oneWord = await call("POST", "/chatbot/knowledge", A, { question: "stock", answer: "CANNED-STOCK-ANSWER" });
      ok("a one-word entry can still be trained", oneWord.status === 201);
      const askStock = await call("POST", "/chatbot/ask", A, { q: "stock of mica tape" });
      ok("a one-word entry does not hijack the live lookup",
        askStock.status === 200 && !/CANNED-STOCK-ANSWER/.test(askStock.d.answer), (askStock.d.answer || "").slice(0, 70));
      // typed back word for word, it is still the right answer
      const askExact = await call("POST", "/chatbot/ask", A, { q: "stock" });
      ok("…but the exact word still returns it", askExact.status === 200 && /CANNED-STOCK-ANSWER/.test(askExact.d.answer), (askExact.d.answer || "").slice(0, 70));
      await call("DELETE", "/chatbot/knowledge/" + oneWord.d.entries[0].id, A);
    }

    /* --- a question made only of stopwords tokenises to nothing, so it could
           never be matched — not even typed back verbatim. --- */
    {
      const stopOnly = await call("POST", "/chatbot/knowledge", A, { question: "who are we", answer: "A cable-tape maker in Bengaluru." });
      ok("a stopword-only question can be trained", stopOnly.status === 201);
      const askIt = await call("POST", "/chatbot/ask", A, { q: "who are we" });
      ok("a stopword-only question answers when typed verbatim",
        askIt.status === 200 && /cable-tape maker/.test(askIt.d.answer), (askIt.d.answer || "").slice(0, 70));
      await call("DELETE", "/chatbot/knowledge/" + stopOnly.d.entries[0].id, A);
    }
    /* --- the oversized-body allowance is for AUTHENTICATED callers only. It is
           mounted ahead of requireAuth, so without a credential gate anyone
           could make the server buffer and parse megabytes before being thrown
           out. Sending 2001 entries proves the body was PARSED (the request is
           rejected on a count the server can only know after parsing) while
           writing nothing. --- */
    {
      const pad = "x".repeat(600);
      const fat = { entries: Array.from({ length: 2001 }, (_, i) => ({ question: "q" + i, answer: pad })) };
      const bytes = JSON.stringify(fat).length;
      ok("the probe body really is over the 1 MB default", bytes > 1024 * 1024, bytes + " bytes");

      const authed = await call("POST", "/chatbot/knowledge", A, fat);
      ok("an authenticated caller may still send a >1 MB training upload",
        authed.status === 400 && /at most 2000/.test(JSON.stringify(authed.d)), authed.status + " " + JSON.stringify(authed.d).slice(0, 80));

      const anon = await call("POST", "/chatbot/knowledge", null, fat);
      ok("the same body WITHOUT a credential is not parsed at 10 MB",
        anon.status !== 400 && (anon.status === 401 || anon.status === 403 || anon.status === 413),
